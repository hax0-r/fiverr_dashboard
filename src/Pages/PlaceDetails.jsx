            ...
            <div className="max-w-[59.4rem] px-5 mx-auto flex items-end mt-10 gap-2">

                {
                    filterData.map(({title},index) => (
                        <p key={index}
                            className={`select-none border-[1.5px] transition-all rounded-full px-4 py-[2.3px] font-medium inline-block cursor-pointer  text-blue border-blue `}>
                            {title}
                        </p>
                    ))
                }
                        ...
                        <div className="max-w-md transition-all animate-myFadeIn" key={item.id}>
                            <div className="border-[1.33px] border-[#cfd1d4] p-3 rounded-xl flex gap-4 flex-col">
                                <div className="relative overflow-hidden rounded-lg">
                                    ...
                                    <p className="bg-blue text-white font-medium p-2 rounded-tr-lg px-3 text-sm absolute bottom-0 left-0"> 27 slot left</p>
                                </div>
                                <div>
                                    <div className="flex justify-between items-center">
                                        <h2 className="text-[#1D1D1D] font-bold text-[1.2rem]">{item.title}</h2>
                                        <p className={`${item.gender === "Male" ? "text-blue bg-[#EFF6FF]" : "text-[#F14242] bg-[#fee2e2]"} inline-block  px-3 py-[6px] rounded-lg`}>{item.gender}</p>
                                    </div>
                                ...
                                <div className=" flex justify-between pb-2 px-1 items-center">
                                    <p className="text-[#FA5501] text-xl  font-bold">
                                        RM 1200/6 month
                                    </p>
